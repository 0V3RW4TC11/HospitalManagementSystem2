...
namespace Tests.Validation
{
    ...
    internal class PatientValidationTests
    {
        ...
        private static void CreateUowAndRepoMock(out Mock<IUnitOfWork> uowMock, out Mock<IRepository<Domain.Entities.Patient>> repoMock)
        {
            uowMock = new();
            repoMock = new();
        ...
        public async Task UpdateValidator_EmailIsUsedByAnotherPatient_ShouldHaveValidationError()
        {
            ...
            CreateUowAndRepoMock(out var uowMock, out var repoMock);
            repoMock.Setup(p => p.AnyAsync(It.IsAny<Specifications.Entity.EntityByIdSpec<Domain.Entities.Patient>>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync(true);
